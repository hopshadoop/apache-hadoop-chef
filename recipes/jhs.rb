...
tmp_dirs   = ["/mr-history", node.apache_hadoop.jhs.inter_dir, node.apache_hadoop.jhs.done_dir, "/tmp", node.hdfs.user_home]
...
node.normal.mr.dirs = [node.apache_hadoop.mr.staging_dir, node.apache_hadoop.mr.tmp_dir, node.hdfs.user_home + "/" + node.apache_hadoop.mr.user]
...
if node.kagent.enabled == "true" 
  kagent_config service_name do
    ...
    log_file "#{node.apache_hadoop.logs_dir}/yarn-#{node.hdfs.user}-#{service_name}-#{node.hostname}.log"
